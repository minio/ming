...
import (
	...
	ming "github.com/minio/ming/cmd"
...
const (
	...
	gcsMinioMultipartPathV1 = ming.GatewayMinioSysTmp + "multipart/v1"
...
EXAMPLES:
  1. Start ming server for GCS backend
  ...
  2. Start ming server for GCS backend with edge caching enabled
     {{.Prompt}} {{.EnvVarSetCommand}} GOOGLE_APPLICATION_CREDENTIALS{{.AssignmentOperator}}/path/to/credentials.json
     {{.Prompt}} {{.EnvVarSetCommand}} MINIO_ROOT_USER{{.AssignmentOperator}}accesskey
     {{.Prompt}} {{.EnvVarSetCommand}} MINIO_ROOT_PASSWORD{{.AssignmentOperator}}secretkey
...
`
	...
	ming.RegisterGatewayCommand(cli.Command{
		Name:               ming.GCSBackendGateway,
...
// Handler for 'ming gcs' command line.
func gcsGatewayMain(ctx *cli.Context) {
	...
	if projectID == "" && os.Getenv("GOOGLE_APPLICATION_CREDENTIALS") == "" {
		...
		cli.ShowCommandHelpAndExit(ctx, ming.GCSBackendGateway, 1)
	}
	...
	ming.StartGateway(ctx, &GCS{projectID})
}
...
func (l *gcsGateway) CleanupGCSMinioSysTmpBucket(ctx context.Context, bucket string) {
	it := l.client.Bucket(bucket).Objects(ctx, &storage.Query{Prefix: ming.GatewayMinioSysTmp, Versions: false})
	for {
		attrs, err := it.Next()
		if err != nil {
...
func (l *gcsGateway) DeleteBucket(ctx context.Context, bucket string, forceDelete bool) error {
	...
	for {
		...
		if objAttrs.Prefix == ming.GatewayMinioSysTmp {
	...
	if gcsMinioPathFound {
		...
		itObject = l.client.Bucket(bucket).Objects(ctx, &storage.Query{Versions: false, Prefix: ming.GatewayMinioSysTmp})
		for {
			objAttrs, err := itObject.Next()
			if err == iterator.Done {
				break
...
func (l *gcsGateway) ListObjects(ctx context.Context, bucket string, prefix string, marker string, delimiter string, maxKeys int) (minio.ListObjectsInfo, error) {
	...
	for {
		...
		for _, attrs := range gcsObjects {
			...
			// Due to ming.GatewayMinioSysTmp keys being skipped, the number of objects + prefixes
			...
			if attrs.Prefix == ming.GatewayMinioSysTmp {
				// We don't return our metadata prefix.
				continue
			}
			if !strings.HasPrefix(prefix, ming.GatewayMinioSysTmp) {
				...
				if strings.HasPrefix(attrs.Prefix, ming.GatewayMinioSysTmp) ||
					strings.HasPrefix(attrs.Name, ming.GatewayMinioSysTmp) {
					continue
				}
			}
...
func (l *gcsGateway) CompleteMultipartUpload(ctx context.Context, bucket string, key string, uploadID string, uploadedParts []minio.CompletePart, opts minio.ObjectOptions) (minio.ObjectInfo, error) {
	...
	gcsMultipartComposeName := func(uploadID string, composeNumber int) string {
		return fmt.Sprintf("%s/tmp/%s/composed-object-%05d", ming.GatewayMinioSysTmp, uploadID, composeNumber)
	}
