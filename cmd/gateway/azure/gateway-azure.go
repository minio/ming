...
const (
	...
	metadataObjectNameTemplate    = ming.GatewayMinioSysTmp + "multipart/v1/%s.%x/azure.json"
	azureMarkerPrefix             = "{minio}"
	metadataPartNamePrefix        = ming.GatewayMinioSysTmp + "multipart/v1/%s.%x"
	maxPartsCount                 = 10000
)
...
EXAMPLES:
  1. Start ming server for Azure Blob Storage backend on custom endpoint.
  ...
  2. Start ming server for Azure Blob Storage backend with edge caching enabled.
     {{.Prompt}} {{.EnvVarSetCommand}} MINIO_ROOT_USER{{.AssignmentOperator}}azureaccountname
     {{.Prompt}} {{.EnvVarSetCommand}} MINIO_ROOT_PASSWORD{{.AssignmentOperator}}azureaccountkey
...
// Handler for 'ming azure' command line.
func azureGatewayMain(ctx *cli.Context) {
	...
	// Validate gateway arguments.
	logger.FatalIf(ming.ValidateGatewayArguments(serverAddr, host), "Invalid argument")

	ming.StartGateway(ctx, &Azure{host})
}
...
func (g *Azure) Name() string {
	return ming.AzureBackendGateway
}
...
func parseStorageEndpoint(host string, accountName string) (*url.URL, error) {
	...
	if host != "" {
		host, secure, err := ming.ParseGatewayEndpoint(host)
		if err != nil {
			return nil, err
		}
...
func (a *azureObjects) ListObjects(ctx context.Context, bucket, prefix, marker, delimiter string, maxKeys int) (result minio.ListObjectsInfo, err error) {
	...
	for len(objects) == 0 && len(prefixes) == 0 {
		...
		for _, blob := range resp.Segment.BlobItems {
			if delimiter == "" && strings.HasPrefix(blob.Name, ming.GatewayMinioSysTmp) {
				// We filter out ming.GatewayMinioSysTmp entries in the recursive listing.
				continue
			}
		...
		for _, blobPrefix := range resp.Segment.BlobPrefixes {
			if blobPrefix.Name == ming.GatewayMinioSysTmp {
				// We don't do strings.HasPrefix(blob.Name, ming.GatewayMinioSysTmp) here so that
...
func (a *azureObjects) ListObjectParts(ctx context.Context, bucket, object, uploadID string, partNumberMarker int, maxParts int, opts minio.ObjectOptions) (result minio.ListPartsInfo, err error) {
	...
	for _, blob := range resp.Segment.BlobItems {
		if delimiter == "" && !strings.HasPrefix(blob.Name, ming.GatewayMinioSysTmp) {
			// We filter out non ming.GatewayMinioSysTmp entries in the recursive listing.
			continue
		}
