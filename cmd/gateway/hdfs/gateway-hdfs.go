...
EXAMPLES:
  1. Start ming server for HDFS backend
  ...
  2. Start ming server for HDFS with edge caching enabled
     {{.Prompt}} {{.EnvVarSetCommand}} MINIO_ROOT_USER{{.AssignmentOperator}}accesskey
     {{.Prompt}} {{.EnvVarSetCommand}} MINIO_ROOT_PASSWORD{{.AssignmentOperator}}secretkey
...
// Handler for 'ming hdfs' command line.
func hdfsGatewayMain(ctx *cli.Context) {
	// Validate gateway arguments.
	if ctx.Args().First() == "help" {
		cli.ShowCommandHelpAndExit(ctx, ming.HDFSBackendGateway, 1)
	}

	ming.StartGateway(ctx, &HDFS{args: ctx.Args()})
}
...
func (g *HDFS) Name() string {
	return ming.HDFSBackendGateway
}
