...
EXAMPLES:
  1. Start ming server for AWS S3 backend
  ...
  2. Start ming server for AWS S3 backend with edge caching enabled
     {{.Prompt}} {{.EnvVarSetCommand}} MINIO_ROOT_USER{{.AssignmentOperator}}accesskey
     {{.Prompt}} {{.EnvVarSetCommand}} MINIO_ROOT_PASSWORD{{.AssignmentOperator}}secretkey
...
// Handler for 'ming s3' command line.
func s3GatewayMain(ctx *cli.Context) {
	...
	logger.FatalIf(ming.ValidateGatewayArguments(serverAddr, args.First()), "Invalid argument")

	// Start the gateway..
	ming.StartGateway(ctx, &S3{args.First()})
}
...
func newS3(urlStr string, tripper http.RoundTripper) (*miniogo.Core, error) {
	...
	endpoint, secure, err := ming.ParseGatewayEndpoint(urlStr)
	if err != nil {
		return nil, err
	}
